...
$sqlheros = "SELECT user_id FROM hero";
$stmtheros = $pdo->query($sqlheros);
$heros = $stmtheros->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT email FROM user WHERE id = :user_id");
    $stmt->bindParam(':user_id', $hero['user_id'], PDO::PARAM_INT); 
    $stmt->execute();
    ...
    if ($email_user) {
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'UN NOUVEAU INCIDENT A ÉTAIT DÉCLARÉ';
            $mail->Body = "<h1></h1><p>Votre demande a été acceptée</p><p>Désormais vous faites partie de la communauté des supers-héros. Vous pouvez à présent intervenir sur les incidents.</p>";
            $mail->AltBody = "Votre demande a été acceptée. Désormais vous faites partie de la communauté des supers-héros. Vous pouvez à présent intervenir sur les incidents.";
...
$mail = new PHPMailer(true);
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Vous avez déclarer un incident';
            $mail->Body = "<h1>Déclaration incident</h1><p>Vous venez de déclaré un incident</p>
            <p><br>Votre incident : <br>
            $type , $vilain , $lieu, $priorite, $description</p>";
            $mail->AltBody = "déclaration incident";
...
echo json_encode(['status' => 'success']);
        ...
        echo json_encode(['status' => 'error', 'message' => 'Erreur lors de la déclaration de l\'incident.']);
        exit();
        } 
    }
}
...
<body>
    <div id="loading"> <!-- From Uiverse.io by Nawsome --> 
<div class="svg-frame">
	<svg style="--i:0;--j:0;">
		<g id="out1">
    ...
    $('#form').submit(function(e) {
        ...
        $.ajax({
            url: 'incident.php',   // Le script PHP qui traite le formulaire
            type: 'POST',
            data: formData,
            success: function(response) {
                // Lorsque le serveur répond (quand l'email est envoyé et l'incident ajouté)
                $('#loading').hide();  // Cacher la barre de chargement

                // Si nécessaire, vous pouvez gérer la réponse ici (ex. message de succès)
                window.location.href = "../index.php";  // Rediriger après le traitement
            },
            error: function() {
                // Si une erreur se produit
                $('#loading').hide();
                alert('Une erreur s\'est produite lors de l\'envoi de l\'incident.');
            }
        });
    });
});
